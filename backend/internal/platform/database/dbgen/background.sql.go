...
const getBackgroundQueueLagSummary = `-- name: GetBackgroundQueueLagSummary :one
WITH outbox_due AS (
  ...
  FROM outbox_events
  WHERE status = 'FAILED'
    AND COALESCE(next_retry_at, available_at) <= $1::timestamptz
    AND attempt_count < max_attempts
  UNION ALL
...
type GetBackgroundQueueLagSummaryRow struct {
	OutboxDuePendingCount                int64
	OutboxDueFailedCount                 int64
	OutboxExpiredProcessingCount         int64
	OutboxOldestDueAt                    pgtype.Timestamptz
	BackgroundJobsDuePendingCount        int64
	BackgroundJobsDueFailedCount         int64
	BackgroundJobsExpiredProcessingCount int64
	BackgroundJobsOldestDueAt            pgtype.Timestamptz
}

func (q *Queries) GetBackgroundQueueLagSummary(ctx context.Context, now pgtype.Timestamptz) (*GetBackgroundQueueLagSummaryRow, error) {
	row := q.db.QueryRow(ctx, getBackgroundQueueLagSummary, now)
	var i GetBackgroundQueueLagSummaryRow
	err := row.Scan(
		&i.OutboxDuePendingCount,
		&i.OutboxDueFailedCount,
		&i.OutboxExpiredProcessingCount,
		&i.OutboxOldestDueAt,
		&i.BackgroundJobsDuePendingCount,
		&i.BackgroundJobsDueFailedCount,
		&i.BackgroundJobsExpiredProcessingCount,
		&i.BackgroundJobsOldestDueAt,
	)
	return &i, err
}
