/*
 File Workshop V1.0 - PostgreSQL 空库初始化脚本
 设计依据: docs/File-Workshop-V1.0-数据库设计说明.md

 Navicat Premium 17 导入方法:
 1. 使用具有 CREATE SCHEMA、CREATE TABLE 权限的账号连接目标 PostgreSQL 数据库。
 2. 打开“查询”，载入本文件，确认连接的是空库后执行整个脚本。
 ...
 7. 本文件仅用于首版空库初始化，不是可重复执行脚本；若任一语句失败，事务整体回滚。
