...
WebOS 工作台是前端 Shell 能力，不是新的后端产品形态。它负责桌面、窗口管理、启动器、Dock/任务栏、通知和快捷入口；业务页面仍按系统模块组织，并复用同一套 Vue Router、OpenAPI 客户端、权限校验、TanStack Vue Query 缓存和 Pinia 状态。

建议将工作台 Shell 与业务模块拆分：

- `app-shell/classic`：传统布局；
- `app-shell/webos`：WebOS 桌面、窗口管理、启动器和任务栏；
- `modules/*`：文件、搜索、上传、共享、审计、后台任务等业务组件。

用户偏好可以保存默认工作台模式。移动端、低性能设备和无障碍场景必须支持回退传统布局。明暗主题必须同时覆盖两种模式。

后续若引入第三方窗口管理、拖拽或 Web Desktop 组件，必须先核对许可证、维护状态、依赖安全和适配成本。
...
- Vitest：组件、工具和状态逻辑；
- Playwright：登录、工作台模式切换、WebOS 窗口打开/关闭、文件上传、权限、分享、搜索、回收站、WebDAV 关键联动等端到端场景。
