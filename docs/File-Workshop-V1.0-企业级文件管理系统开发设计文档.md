...
两种模式共享同一套路由、OpenAPI 客户端、权限判定、业务组件和状态管理，不为 WebOS 复制第二套业务实现。WebOS 只作为前端 Shell，提供桌面、启动器、任务栏、多窗口、通知和快捷入口；不新增后端领域模型，不改变数据库结构、权限语义、审计规则或 API 契约。

用户可以在个人偏好中切换工作台模式。移动端、低性能设备、无障碍模式或企业统一策略可以默认使用传统布局。所有模式必须支持明暗主题。
...
```text
┌─────────────────────────────────────────────────────────┐
│ 顶部状态栏：全局搜索 / 通知 / 任务状态 / 用户           │
├─────────────────────────────────────────────────────────┤
│ 桌面区域：文件管理器、搜索、上传中心、共享、审计等窗口  │
│                                                         │
│   ┌───────────────┐     ┌──────────────────────────┐    │
│   │ 文件管理器    │     │ 预览 / 版本 / 属性        │    │
│   └───────────────┘     └──────────────────────────┘    │
│                                                         │
├─────────────────────────────────────────────────────────┤
│ Dock / 任务栏：启动器、已打开窗口、上传队列、后台任务   │
└─────────────────────────────────────────────────────────┘
```

WebOS 首批应用入口包括：文件管理器、搜索、上传中心、共享中心、回收站、预览窗口、版本历史、后台任务、审计中心和个人设置。窗口状态只属于客户端偏好，不得作为业务事实来源。
...
WebOS 模式还必须保证窗口焦点、层级、拖拽、缩放、最小化、最大化、快捷键和任务栏状态具备键盘可达路径；无法满足时必须提供传统布局回退。
