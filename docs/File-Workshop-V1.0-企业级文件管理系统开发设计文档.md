...
法定保留不属于上述状态枚举，而是独立、可多条并存、可审计的 Legal Hold 记录。任一活动 Hold 都阻止其作用范围内的永久清理，但不覆盖文件当前生命周期或扫描状态。
...
冲突时采用更严格的保留要求。法定保留由独立 Legal Hold 记录设置和解除，不作为可被普通生命周期策略自动开启或覆盖的布尔配置。
...
| POST | `/documents/{id}/legal-hold` | 设置保留冻结 |
| DELETE | `/documents/{id}/legal-hold` | 解除冻结 |
...
法定保留：由独立 Legal Hold 记录表达，不属于文件状态枚举
